...
namespace Frap3Core.BCore
{
    ...
    ///	<para>修正内容</para>
    ///	</remarks>
    public class PostgreSQLDataAccessHelper : AbstractDataAccessHelper
    {
        #region コンストラクタ
        ...
        public PostgreSQLDataAccessHelper(ConnectionInfo connectInfo)
        {
        ...
        public override DbConnection GetConnection()
        {
            ...
            try
            {
                dbConnection = PostgreSQLUtility.GetConnection(user, password, server, database, port.ToString());
            }
            catch (Exception)
            {
                dbConnection = PostgreSQLUtility.GetConnection(user, password, server, database);
            }

            PostgreSQLUtility.OpenConnection((NpgsqlConnection)dbConnection);
        ...
        {
            PostgreSQLUtility.CloseConnection((NpgsqlConnection)connection);
        }
        ...
        {
            NpgsqlCommand cmd = new NpgsqlCommand(sql, (NpgsqlConnection)con);
        ...
        public override DbCommand CreateCommand(string sql, DbConnection con, DbTransaction txn)
        {
            ...
            if (txn != null)
            {
                cmd.Transaction = (NpgsqlTransaction)txn;
            }
        ...
        {
            return PostgreSQLUtility.CreateParameter(name, (NpgsqlDbType)this.ConvertDbType(type), direction, value);
        }
        ...
        {
            return PostgreSQLUtility.CreateParameter(name, (NpgsqlDbType)this.ConvertDbType(type), direction, size, value);
        }
        ...
        {
            return PostgreSQLUtility.CreateParameter(name, (NpgsqlDbType)this.ConvertDbType(type), direction, scale, value);
        }
        ...
        public override int Fill(DbCommand dbCommand, DataSet dataSet, string tableName)
        {
            ...
            adapter.SelectCommand = (NpgsqlCommand)dbCommand;
        ...
        {
            NpgsqlDbType newType;

            switch (type)
            {
                case Frap3DBType.Char:
                    // 固定長文字列型（2000バイト）
                    newType = NpgsqlDbType.Char;
                    break;
                ...
                case Frap3DBType.VarChar:
                    // 文字列型（4000バイトまで）
                    newType = NpgsqlDbType.Varchar;
                    break;
                case Frap3DBType.SmallInt:
                    // 16ビット符号付き整数
                    newType = NpgsqlDbType.Smallint;
                    break;
                case Frap3DBType.Int:
                    // 32ビット符号付き整数型
                    newType = NpgsqlDbType.Integer;
                    break;
                case Frap3DBType.Float:
                    // 単精度浮動小数型
                    newType = NpgsqlDbType.Real;
                    break;
                case Frap3DBType.Double:
                    // 倍精度浮動小数型
                    newType = NpgsqlDbType.Double;
                    break;
                ...
                case Frap3DBType.Decimal:
                    // 数値型
                    newType = NpgsqlDbType.Numeric;
                    break;
                case Frap3DBType.Bit:
                    // 1ビット型
                    newType = NpgsqlDbType.Bit;
                    break;
                //break;
                case Frap3DBType.DateTime:
                    //日付型(TimeStamp)
                    newType = NpgsqlDbType.Timestamp;
                    break;
                case Frap3DBType.BLOB:
                    // バイナリデータを格納
                    newType = NpgsqlDbType.Bytea;
                    break;
                case Frap3DBType.CLOB:
                    // 文字データを格納
                    newType = NpgsqlDbType.Text;
                    break;
        ...
        public override bool CheckInsertError(Exception ex)
        {
            ...
            {
                return ((NpgsqlException)ex).ErrorCode == 23505;
            }
            else
            {
                return false;
